...
For energy efficiency, students should consider the different types of houses and find that newer buildings are more efficient and that some buildings have also been upgraded between 2014 and 2018. There are multiple aspects that could be considered, such as the change in number

```{r}
Houses <- read.csv( "Utopia Energy Efficiency.csv" )

Houses %>% group_by( ERC2014 ) %>%
  summarise( Number = n() ) %>%
  filter( is.na(ERC2014) == FALSE )
Houses %>% group_by( ERC2018 ) %>%
  summarise( Number = n() )
```

Some more details could be done on the different types of property, although the data wrangling becomes more complex.

### Marking guidance

For an answer to be considered first class, it has to explore all four aspects mentioned above: (i) change over the years, (ii) variation throughout the year, (iii) variation throughout the day and (iv) improvement in energy efficiency. A solution may achieve 13 or more marks if it provides an in-depth discussion which sets the analysis in context, or presents an elegant solution in terms of data visualization, e.g.. instead of just plotting the hourly average, one may consider the hourly demand relative to daily demand. 

The following list outlines some scenarios and the mark they should achieve:

* **11 marks** - The report performs the analysis as presented above, but it ignores the trend. For instance, seasonality is explored but the data for all years is considered, while it would be better to just focus on recent years.

* **9-10 marks** - The analysis considers three of the four aspects above and has a good structure/presentation, including clear conclusions.

* **9-10 marks** - The report analyses all aspects above, but there are weaknesses in terms of interpretation and ignores the trends in the data.

* **8 marks** - The report addresses three of the four points in a reasonable manner, with weaknesses in the clarity of the presentation/interpretation.

* **6-7 marks** - Two of the foud aspects have been studied in a nice level of detail. 

* **6 marks** - The report addresses two of the three points, but there are some major weaknesses in terms of presentation and interpretation.

* **at most 6 marks** - An answer that lacks any interpretation of plots and results.
