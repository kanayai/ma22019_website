...
```{=html}
<script>
// Safety Check: Redirect if on the incorrect personal GitHub Pages site
if (window.location.hostname.includes("kanayai.github.io")) {
  document.body.innerHTML = `
    <div style="margin: 50px auto; max-width: 800px; text-align: center; font-family: sans-serif;">
      <h1 style="color: #c62828;">⚠️ Wrong Website Link</h1>
      <p style="font-size: 1.2em;">You have accessed an outdated or incorrect version of the setup guide.</p>
      <p>Please use the official course website:</p>
      <br>
      <a href="https://github.bath.ac.uk/pages/ma22019-2026/ma22019_website/computing_setup/setup.html" 
         style="background-color: #005871; color: white; padding: 15px 30px; text-decoration: none; border-radius: 5px; font-weight: bold; font-size: 1.1em; display: inline-block;">
         Go to Official Setup Guide
      </a>
    </div>
  `;
}
...
*   Just **log in** to a PC.
*   Then [jump straight to **Section 2: Your Computational Toolkit**](#your-computational-toolkit).
...
3.  Copy and paste the code below into the new file. (Tip: You can use the copy icon 📋 in the top-right of the code block).
4.  **Save the file** as `install_packages.R` in your `MA22019` folder.
5.  Select all the code (Cmd+A / Ctrl+A).
...
1.  **Re-installing Packages**: R packages are often stored in a temporary location (`C:/Users/.../AppData/`). This folder is **wiped** when you log off or switch computers.
    *   **The Fix**: Keep the `install_packages.R` script handy! You may need to run it again at the start of a session if your packages are missing.

2.  **"Rtools is Missing" Warning**: When running the install script, you might see a scary red warning about `Rtools` not being installed.
    *   **The Fix**: **Ignore it.** Rtools is for compiling packages from source. We use pre-built binaries, so you don't need it. As long as you see `Setup Complete` at the end, you are fine!
...
*   **Your Use:** Think of it as your **Sandbox**. Want to test some code from a lecture? Want to try a new package? create a script in `practice/my_experiments.R` for example! It's the one place where you can be messy without breaking anything important.
...
1. Open RStudio (not inside any project! Don't know what a project is? See Section 7 below)
...
5. Click **New Folder** → type `practice`
6. Click **New Folder** → type `homework`
7. Click **New Folder** → type `coursework`
8. (Do **not** create `materials` yet — RStudio will do it for you!)
...
**Storage Limits**: Your H: Drive ("MyFiles") is safe and persistent, but it only has **8GB** of space.
    *   **Advice**: Keep your active code projects on H:, but move large datasets or old projects to your OneDrive or a USB stick if you run out of space.
:::
...
3. **Project directory name**: Leave as `materials` (this creates the folder for you!)
4. **Create project as subdirectory of**: Browse and select your `MA22019` folder
...
- ✅ The **Files** pane should show folders like `lab_solutions/`, `labs/`, `live_coding`
...
1.  **Find your repo URL**: 
    - Go to `https://github.bath.ac.uk/ma22019-2026/lab-00-yourusername`
...
2.  **Clone it in RStudio**:
    ...
    - **Project directory name**: Leave this as `lab-00-yourusername`.
    - **Create project as subdirectory of**: Click **Browse** and navigate to `.../MA22019/practice`.
    - Click **Create Project**.

3.  **Verify**:
    - RStudio should open your new project.
    - Look in the **Files** pane (bottom-right). You should see a file called `lab_00.qmd`.
    - Click on `lab_00.qmd` to open it.
